...
from core.train_attention import train, train_stage1, train_stage0
...
def main(config):
    ...
    if config.mode == 'train':
        ...
        with torch.no_grad():
            for i, data in enumerate(loaders.get_train_normal_with_shape_loader()):
                ...
                rgb_shape, ir_shape = data[4].to(model.device), data[5].to(model.device)

                rgb_map = model.model(x1=rgb_imgs, get_map=True)
                ir_map = model.model(x2=ir_imgs, get_map=True)
                rgb_shape_map = model.model(x1=rgb_shape, get_map=True)
                ir_shape_map = model.model(x2=ir_shape, get_map=True)

                pids = torch.cat([rgb_pids, ir_pids], dim=0)
                image_map = torch.cat([rgb_map, ir_map], dim=0)
                shape_map = torch.cat([rgb_shape_map, ir_shape_map], dim=0)

                for i, img_map, sh_map in zip(pids, image_map, shape_map):
                    labels.append(i)
                    image_maps.append(img_map.cpu())
                    shape_maps.append(sh_map.cpu())

            labels_list = torch.stack(labels, dim=0).cuda()
            image_maps_list = torch.stack(image_maps, dim=0).cuda()
            shape_maps_list = torch.stack(shape_maps, dim=0).cuda()
            batch = config.batch_size * 2
            num_image = labels_list.shape[0]
            i_ter = num_image // batch
        del labels, image_maps, shape_maps
        ...
        for current_epoch in range(start_train_epoch, config.stage1_train_epochs):
            ...
            _, result = train_stage0(model, num_image, i_ter, batch, labels_list,
                                     image_maps_list, shape_maps_list)
