...
def train_stage1_3share(base, num_image, i_ter, batch, visible_labels_list, visible_image_features_list, infrared_labels_list, infrared_image_features_list):
    base.set_train()
    meter = MultiItemAverageMeter()
    iter_list = torch.randperm(num_image).to(base.device)
    for i in range(i_ter):
...
def train_2rgb(base, loaders, text_features, config):
    ...
    for iter, (rgb_1, rgb_2, ir_1, rgb_pids, ir_pids) in enumerate(loader):
        ...
        rgb_ir_part_features = torch.cat([rgb_part_features, ir_part_features], dim=1)
        ...
        for i in range(num_part):
            ...
            loss_ipc += base.IPC_1(rgb_ir_part_features[i], rgb_pids)


        loss_ipc /= num_part
        # loss_ipd = base.IPD(per_part_features, rgb_pids)
        loss_ipd = base.IPD_1(rgb_ir_part_features, rgb_pids)
